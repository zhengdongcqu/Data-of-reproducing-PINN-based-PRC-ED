...
casename =  'RTS79';
[baseMVA,bus,busPmax,busPmin,busQmax,busQmin,busPG,busQG,gen,branch, PBase ,PFBase,busLoadImportant] = feval(casename);
...
branch(:,6)=0.75*branch(:,6)/baseMVA;
bus(:,3)=bus(:,3)/baseMVA;
gen(:,2)=1.1*gen(:,2)/baseMVA;
gen(Set_Fast,2)=2.2;

% lambda_line=0.001+(0.005-0.001)*rand(Nl,1);
% lambda_gen=0.01+(0.05-0.01)*rand(Ng,1);

load('lambda_gen_RTS79.mat')
load('lambda_line_RTS79.mat')

Set_RE=[9,10,11,20,21];
N_RE=length(Set_RE);

Set_TH=setdiff([1:Ng]',Set_RE);
N_TH=length(Set_TH);

sum(gen(Set_RE,2))/sum(gen(:,2))
sum(gen(Set_TH,2))/sum(bus(:,3))

for i=1:Nb
    pgmax=sum(gen(gen(:,1)==i,2));
    for j=1:Nl
...
up_cost=500+500*gen(:,2)/max(gen(:,2));
shut_cost=up_cost/2;
T_on_off=ceil(6*gen(:,2)/max(gen(:,2)));
T_on_off(Set_Fast)=1;

Nt=24;

Pgmin=repmat(0.20*gen(:,2),1,Nt);
Pgmin(Set_RE,:)=0;
...
% Set_Pgmax=cell(K,1);
% for k=1:K
%     Pgmax=repmat(gen(:,2),1,Nt);
%     Pgmax(Set_RE,:)=repmat(rep_hourly_wind_output(k,:),N_RE,1).*repmat(gen(Set_RE,2),1,Nt);
%     Set_Pgmax{k}=Pgmax;
% end
% Pgmax=Set_Pgmax{4};
% 
...
% %爬坡约束
% for t=1:Nt-1
%     Constraints_Clearing0=[Constraints_Clearing0,[PG(Set_TH,t+1)-PG(Set_TH,t)<=Ramp_max(Set_TH).*UG(Set_TH,t)+(1-UG(Set_TH,t)).*gen(Set_TH,2)]];
%     Constraints_Clearing0=[Constraints_Clearing0,[PG(Set_TH,t)-PG(Set_TH,t+1)<=Ramp_max(Set_TH).*UG(Set_TH,t+1)+(1-UG(Set_TH,t+1)).*gen(Set_TH,2)]];
% end
% 
% %爬坡约束
...
% 
% save('RTS79_da_result_TL.mat','da_result')

load('RTS79_da_result_TL.mat')
UG_da=da_result{1};
...
idx=[];
for root=2:N_failure
    gen_failure_num=failure_set{1,1}{root,1};
    if isempty(gen_failure_num)
        idx=[idx;root];
    end
...
num_selected_failure=setdiff([1:N_failure]',idx);
N_selected_failure=length(num_selected_failure);

prob_failure_up=prob_failure(num_selected_failure);
pg_state_up=pg_state(num_selected_failure,:);

% 概率合并
map_up = containers.Map('KeyType', 'char', 'ValueType','double' );
for i=1:N_selected_failure
    key=num2str(pg_state_up(i,:));
    map_up(key)=i;
end

for i=1:N_failure
    gen_failure_num=failure_set{1,1}{i,1};
    if isempty(gen_failure_num)
        continue
    end
    num=intersect(gen_failure_num,num_down);
    if ~isempty(num)
        tmp=pg_state(i,:);
        tmp(num_down)=1;
        key=num2str(tmp);
        prob_failure_up(map_up(key))=prob_failure_up(map_up(key))+prob_failure(i);
    end
end
% sum(prob_failure_up)+sum(prob_failure(all(pg_state == 1, 2), :))-prob_failure(1)
...
for t=1:Nt
    for s_each_t = 1:Ns_each_t
        ...
        PGMAX(Set_RE, s) = min(Pgmax(Set_RE, t) .* (1 - 0.30 + 0.60 * x(N_Fast+N_TH+1:N_Fast+Ng, s)),gen(Set_RE,2));
...
while(1)
    ...
    for s=1:N_add
        ...
        PGMAX_add(Set_RE, s) = min(Pgmax(Set_RE, t) .* (1 - 0.30 + 0.60 * x_tmp(N_Fast+N_TH+1:N_Fast+Ng, s)),gen(Set_RE,2));
...
% [dataX_fake,txt,raw] = xlsread('生成数据汇总.xlsx',2); 
% PGMAX_train = dataX_fake(:,1:Ng)';
% PLMAX_train = dataX_fake(:,Ng+1:end)';
% Ns = size(PGMAX_train,2);

xita=1e-8;

EDNS_vector=zeros(Ns,1);
LOLP_vector=zeros(Ns,1);
all_load_shedding_matrix=zeros(Ns,N_failure);

partial_derivative_matrix_EDNS=zeros(Ns,Ng+Nb);
partial_derivative_matrix_LOLP=zeros(Ns,Ng+Nb);

N_parpool=8;%调用的处理器个数
...
for batch=1:ceil(N_failure/N_batch)
    ...
    all_load_shedding_matrix(:,failure_num)=load_shedding_matrix';
...
all_load_shedding_matrix=all_load_shedding_matrix(:,num_selected_failure);


% EDNS_taylor_vector = EDNS_vector + sum(partial_derivative_matrix_EDNS .* (x - [PGMAX_train;PLMAX_train]'),2)

% L_est = estimate_Lipschitz_constant(PGMAX_train, PLMAX_train, partial_derivative_matrix_EDNS);
% PGMAX_test = PGMAX_train * (0.95 + 0.05 *rand);
% PLMAX_test = PLMAX_train * (0.95 + 0.05 *rand);
% [error_bound, avg_dist] = compute_theoretical_error_bound(PGMAX_test, PLMAX_test, PGMAX_train, PLMAX_train, L_est);
% 
...
% 
% load_shedding_matrix_gs=repmat(sum(PLMAX_train,1),N_selected_failure,1)-pg_state_up*PGMAX_train;
% load_shedding_matrix_gs(load_shedding_matrix_gs<0)=0;
% EDNS_vector_gs_test=load_shedding_matrix_gs'*prob_failure_up;
% LOLP_vector_gs_test=(load_shedding_matrix_gs>0)'*prob_failure_up;
% 
% test1 = EDNS_vector_gs-EDNS_vector_gs_test
% test2 = LOLP_vector_gs-LOLP_vector_gs_test
...
% N_fd=20;
% [idx,edges]=kmeans(LOLP_vector, N_fd+1);
% edges=sort(edges);
% % edges=linspace(min(LOLP_vector),max(LOLP_vector),N_fd+1);
% LOLP_edge_mi=zeros(N_fd,1);
% error=zeros(size(LOLP_vector));
% idx_LOLP=zeros(size(LOLP_vector,1),N_fd);
% for fd=1:N_fd
%     num=find(LOLP_vector>=edges(fd) & LOLP_vector<edges(fd+1));
%     idx_LOLP(num,fd)=1;
%     LOLP_fd=mean(edges(fd:fd+1));
%     LOLP_edge_mi(fd)=LOLP_fd;
%     error(num)=abs(LOLP_fd-LOLP_vector(num))./LOLP_vector(num);
% end
% sum(idx_LOLP,1)
% mean(error)
...
idx_LOLP=zeros(size(LOLP_vector,1),K);
for k=1:K
    num=find(idx==k);
    idx_LOLP(num,k)=1;
end
...
% 为每个簇分配不同颜色
colors = hsv(K); % 使用HSV色系生成K种不同颜色
% 绘制每个簇的数据点
for i = 1:K
    cluster_data = data(idx == i, :);
    scatter(cluster_data(:, 1), cluster_data(:, 2), 50, colors(i, :), ...
           'filled', 'MarkerEdgeColor', 'k', 'LineWidth', 0.5);
    hold on;
end
% 绘制聚类中心
scatter(ctr(:, 1), ctr(:, 2), 200, 'k', 'x', 'LineWidth', 3);
scatter(ctr(:, 1), ctr(:, 2), 150, 'w', '+', 'LineWidth', 2);

% load('ED_RTS79_data_for_testing_PINN_750.mat','LOLP_vector')
% idx_LOLP_test=zeros(size(LOLP_vector,1),N_fd);
% for fd=1:N_fd
%     num=find(LOLP_vector>=edges(fd) & LOLP_vector<edges(fd+1));
%     idx_LOLP_test(num,fd)=1;
%     LOLP_fd=mean(edges(fd:fd+1));
%     LOLP_edge_mi(fd)=LOLP_fd;
%     error(num)=abs(LOLP_fd-LOLP_vector(num))./LOLP_vector(num);
% end
...
save('ED_RTS79_data_for_training_PINN_test_500_TL_imp.mat')
