...
mat = scipy.io.loadmat('D:\PycharmProjects\pythonProject\Illinois_pinn-based-uc_and_ed20260416\ED_illinois200_data_for_training_PINN_500_TL.mat')
...
# mat = scipy.io.loadmat('D:\PycharmProjects\pythonProject\RTS79_pinn-based-uc_and_ed20260404\ED_RTS79_data_for_training_PINN_test_5000.mat')
# dataYx0_5000 = mat['dataYx0']
# del mat
...
# 读取验证集
mat = scipy.io.loadmat('D:\PycharmProjects\pythonProject\Illinois_pinn-based-uc_and_ed20260416\ED_illinois200_data_for_testing_PINN_1000_TL.mat')
...
print(model)
model.load_state_dict(torch.load('D:\PycharmProjects\pythonProject\Illinois_pinn-based-uc_and_ed20260416\ed_pinns_EDNS_test_2000.pt'))
...
#         loss_bc = 10 * torch.mean(torch.sum(torch.abs(pred_u_x - u_x), dim=1)) + 0.01 * torch.mean(torch.sum((pred_x2 - pred_x3)**2, dim=1))
...
#
# torch.save(model.state_dict(), 'D:\PycharmProjects\pythonProject\Illinois_pinn-based-uc_and_ed20260416\ed_pinns_EDNS_test_500_TL.pt')

model = PINNs_CONV(in_dim=Nb+Ng, hidden_dim=hidden_dim, out_dim=1, failure_matrix=failure_matrix, prob_failure=prob_failure, type=1).to(device)
model.load_state_dict(torch.load('D:\PycharmProjects\pythonProject\Illinois_pinn-based-uc_and_ed20260416\ed_pinns_EDNS_test_500_TL.pt'))
...
tmp = np.abs(train_real-train_pred) / train_real
...
with pd.ExcelWriter('illinois200_pinn_training_EDNS_output_test_500_TL.xlsx') as writer:
