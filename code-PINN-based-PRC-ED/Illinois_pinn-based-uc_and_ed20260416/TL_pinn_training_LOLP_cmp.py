...
# dataZx0 = dataZx0[shuffled_indices, :]
...
#
# torch.save(model.state_dict(), 'D:\PycharmProjects\pythonProject\Illinois_pinn-based-uc_and_ed20260416\ed_pinns_LOLP_test_500_TL_cmp.pt')

model = PINNs_CONV(in_dim=Nb+Ng, hidden_dim=hidden_dim, out_dim=1, failure_matrix=failure_matrix, prob_failure=prob_failure, type=2).to(device)
model.load_state_dict(torch.load('D:\PycharmProjects\pythonProject\Illinois_pinn-based-uc_and_ed20260416\ed_pinns_LOLP_test_500_TL_cmp.pt'))
...
with pd.ExcelWriter('illinois200_pinn_training_LOLP_output_test_500_TL_cmp.xlsx') as writer:
