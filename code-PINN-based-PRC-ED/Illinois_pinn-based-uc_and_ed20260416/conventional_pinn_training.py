...
dataX_train = torch.tensor(dataX.squeeze() , dtype=torch.float, requires_grad=True).to(device)
dataY_train = torch.tensor(dataY.squeeze() , dtype=torch.float).to(device)
dataZ_train = torch.tensor(dataZ.squeeze() , dtype=torch.float).to(device)
dataYx0_train = torch.tensor(dataYx0.squeeze(), dtype=torch.float).to(device)
dataZx0_train = torch.tensor(dataZx0.squeeze() , dtype=torch.float).to(device)
...
# 读取验证集
mat = scipy.io.loadmat('D:\PycharmProjects\pythonProject\Illinois_pinn-based-uc_and_ed20260416\ED_illinois200_data_for_testing_PINN_300.mat')
...
total_epochs = 1000
warmup_epochs = 0.2 * total_epochs
balance_epochs = total_epochs - warmup_epochs
...
#         # 第二阶段:退火
#         # # 线性衰减
#         # progress = (balance_epochs + warmup_epochs - i) / balance_epochs
#         # w_bc = 0.01 + 0.49 * progress
#
#         # 指数衰减
#         progress = (i - warmup_epochs) / balance_epochs
#         k = 5.0  # 可调整
#         w_bc_start = 0.05
#         w_bc_end = 0.001
#         w_bc = w_bc_end + (w_bc_start - w_bc_end) * math.exp(-k * progress)
#
#     def closure():
#         model.train()
#         pred_x1, pred_x2 = model(x)
#
#         pred_u_x = torch.autograd.grad(pred_x1.sum(), x, grad_outputs=torch.ones_like(pred_x1.sum()), retain_graph=True, create_graph=True)[0]
#         pred_u_z = torch.autograd.grad(pred_x2.sum(), x, grad_outputs=torch.ones_like(pred_x2.sum()), retain_graph=True, create_graph=True)[0]
#
#         phy_loss_edns = torch.mean(torch.sum(huber(pred_u_x - u_x), dim=1))
#         cosine_sim = F.cosine_similarity(pred_u_z, u_x, dim=1)
#         phy_loss_lolp = torch.mean(1 - cosine_sim)
#
#         loss_res = (F.l1_loss(pred_x1, y) + F.l1_loss(pred_x2, z))
#         loss_bc = 10 * phy_loss_lolp + phy_loss_edns
#
#         lambda_l2 = 1e-3
...
#
#         loss_sum = loss_res + w_bc * loss_bc + lambda_l2 * l2_reg
#
...
#
# torch.save(model.state_dict(), 'D:\PycharmProjects\pythonProject\Illinois_pinn-based-uc_and_ed20260416\ed_conventional_pinns_test_2000_seed2.pt')
...
with pd.ExcelWriter('D:\PycharmProjects\pythonProject\Illinois_pinn-based-uc_and_ed20260416\illinois200_conventional_pinn_training_output_test_2000_seed2.xlsx') as writer:
