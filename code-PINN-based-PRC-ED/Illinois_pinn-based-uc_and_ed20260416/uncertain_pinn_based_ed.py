...
model_edns.load_state_dict(torch.load('D:\PycharmProjects\pythonProject\Illinois_pinn-based-uc_and_ed20260416\ed_pinns_EDNS_test_2000.pt'))
...
model_lolp.load_state_dict(torch.load('D:\PycharmProjects\pythonProject\Illinois_pinn-based-uc_and_ed20260416\ed_pinns_LOLP_test_2000.pt'))
for param in model_lolp.parameters():
    param.requires_grad = False

mat = scipy.io.loadmat('D:\PycharmProjects\pythonProject\Illinois_pinn-based-uc_and_ed20260416\illinois200_rolling_ed_K4.mat')
...
while True:
    ...
    # for t in range(Nt):
        ...
        #     x_grad_EDNS_tensor = torch.autograd.grad(EDNS_t_s, x, grad_outputs=torch.ones_like(EDNS_t_s), retain_graph=True, create_graph=True)[0]
        #     LOLP_t_s, _, _ = model_lolp(x)
        #     x_grad_LOLP_tensor = torch.autograd.grad(LOLP_t_s, x, grad_outputs=torch.ones_like(LOLP_t_s), retain_graph=True, create_graph=True)[0]
        #     EDNS_t_s = EDNS_vector_std * EDNS_t_s.to(device).cpu().detach().numpy().squeeze() + EDNS_vector_mean
        #     LOLP_t_s = LOLP_vector_std * LOLP_t_s.to(device).cpu().detach().numpy().squeeze() + LOLP_vector_mean
        #     EDNS_t_Ns[s, 0] = EDNS_t_s.item()
        #     LOLP_t_Ns[s, 0] = LOLP_t_s.item()
        #     x_grad_EDNS_t_Ns[s, :] = EDNS_vector_std * x_grad_EDNS_tensor.T.to(device).cpu().detach().numpy().squeeze()
        #     x_grad_LOLP_t_Ns[s, :] = LOLP_vector_std * x_grad_LOLP_tensor.T.to(device).cpu().detach().numpy().squeeze()
        #
        # EDNS_t = rep_prob_Forecast_Error.T @ EDNS_t_Ns
        # LOLP_t = rep_prob_Forecast_Error.T @ LOLP_t_Ns
        # x_grad_EDNS_t = rep_prob_Forecast_Error.T @ x_grad_EDNS_t_Ns
        # x_grad_LOLP_t = rep_prob_Forecast_Error.T @ x_grad_LOLP_t_Ns
        #
        # Set_EDNS[t,1] = EDNS_t.item()
        # Set_LOLP[t,1] = LOLP_t.item()
        # # Benders Cuts
        # if EDNS_t > EDNS_Target:
        #     m.addConstr(EDNS_t - EDNS_Target + x_grad_EDNS_t[:,0:Ng].reshape(-1, 1).T @ (thermal_output[:, t] + thermal_reserve[:, t] - (thermal_output_current + thermal_reserve_current)) <= yita_EDNS[:,t])
        # if LOLP_t > LOLP_Target:
        #     m.addConstr(LOLP_t - LOLP_Target + x_grad_LOLP_t[:,0:Ng].reshape(-1, 1).T @ (thermal_output[:, t] + thermal_reserve[:, t] - (thermal_output_current + thermal_reserve_current)) <= yita_LOLP[:,t])
...
filename = 'illinois200_uncertain_pinn_based_ed_results_K4.xlsx'
